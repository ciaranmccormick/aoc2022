...
class Item:
    ...
    def new_worry_level(self, operation: str) -> int:
        old = self.value
        left, op, right_str = operation.split(" ")
...
class Monkey:
    ...
    def from_input(cls, lines: list[str]):
        ...
        for pos, value in enumerate(re.findall(ITEMS, lines[1])[-1].split(", ")):
            items.append(Item(name=f"{number}{pos}", value=int(value)))
    ...
    for i in range(20):
        for monkey in monkeys:
            ...
            while monkey.items:
                ...
                new = item.new_worry_level(monkey.operation)

                item.value = new
                if new % test.condition == 0:
